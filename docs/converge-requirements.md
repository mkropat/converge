...
`bin/converge` is a loop that runs autonomous agent iterations until a project satisfies its requirements documents.
...
- **Bug tracker**: `bin/cbugs`, which keeps the bugs of the repository in the state directory. See `docs/cbugs-requirements.md`.
...
- **State directory**: the durable directory that holds the guidance file, the journal, the bug database, and the logs of a repository. It survives a restart of the machine.
- **Runtime directory**: the temporary directory that holds the locks and the pipes of a repository. It holds nothing that must survive a restart of the machine.
...
- **CV-INV-2**: The script must operate on the repository in the current working directory.
...
- **CV-SCOPE-2**: The script must resolve each path of the scope against the current working directory, and must stop with an error when the path does not exist. The error must name the path. A typo must not become a run that ignores the scope.
- **CV-SCOPE-3**: The loop must give each path to an agent as the operator wrote it. An agent runs in the repository, so a relative path works there.
- **CV-SCOPE-4**: When the scope is empty, the worker prompt and the reviewer prompt must name the requirements documents in general terms, and must instruct the agent to find them in the repository. This is the behavior of a run with no positional argument.
...
- **CV-HARN-1**: The script must select the harness from the first of these that it finds:
  ...
  2. The harness that the state directory records for this repository.
...
- **CV-BG-2**: The client must start a loop only when no loop runs for the repository. In all other cases it must attach to the loop that runs.
- **CV-BG-3**: To attach again, the operator runs `converge` again in the same directory. The client must need no argument for this.
- **CV-BG-4**: One repository must never have two loops. The test for a running loop must be a lock that the operating system releases when the loop dies. A file that a dead loop leaves behind must not block a new run.
...
- **CV-ITER-3**: The worker prompt must instruct the worker to:
  ...
  - **CV-ITER-3.2**: Read the requirements documents of the scope, as "Requirements scope" gives. With an empty scope, find and read the requirements documents in the repository.
...
- **CV-REV-7**: On the first pass of a repository, the range is the whole history. The reviewer must judge how far back to read, and the prompt must tell it that the recent commits matter most.
...
- **CV-STATE-3**: `<name>` must derive from the path of the repository and must be stable. A new run in the same repository must find the state of prior runs. This makes a restart seamless: the guidance file and the journal carry over.
- **CV-STATE-4**: `<name>` must also contain the name of the repository directory, so that an operator can read the directory listing.
...
- **CV-STATE-7**: The state directory must record the harness of the last run. A later run in the same repository uses that harness when the operator gives no `--harness` option.
...
- **CV-ID-1**: The loop must derive `<name>` from the absolute path of the repository. A repository that the operator moves or renames therefore gets a new state directory, and loses the guidance and the journal of the prior runs.
...
- **CV-OOS-23** — **A scope that the state directory records**: rejected. The harness is a property of a repository, and carries over. A scope is the answer to "what do I work on now", which changes from one run to the next. A run with no positional argument therefore works on the whole project, and never on a scope that an earlier run left behind.
